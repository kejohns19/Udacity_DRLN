...
# play a game and display the animation
# nrand = number of random steps before using the policy
def play4(env, policy, time=2000, preprocess=None, nrand=5):
    env.reset()
    ...
    for _ in range(nrand):
        frame1, reward1, is_done, _ = env.step(np.random.choice([RIGHT,LEFT]))
        frame2, reward2, is_done, _ = env.step(0)
        frame3, reward3, is_done, _ = env.step(0)
        frame4, reward4, is_done, _ = env.step(0)
    ...
    for _ in range(time):
        ...
        frame1, _, is_done, _ = env.step(action)
        frame2, _, is_done, _ = env.step(0)
        frame3, _, is_done, _ = env.step(0)
        frame4, _, is_done, _ = env.step(0)

        if preprocess is None:
            anim_frames.append(frame1)
            anim_frames.append(frame3)
        else:
            anim_frames.append(preprocess(frame1))
            anim_frames.append(preprocess(frame3))
...
# collect trajectories for a parallelized parallelEnv object
def collect_trajectories4(envs, policy, tmax=200, nrand=5):
    ...
    for _ in range(nrand):
        fr1, re1, _, _ = envs.step(np.random.choice([RIGHT, LEFT],n))
        fr2, re2, _, _ = envs.step([0]*n)
        fr3, re3, _, _ = envs.step([0]*n)
        fr4, re4, _, _ = envs.step([0]*n)
    ...
    for t in range(tmax):
        ...
        # shape (n, 2, 80, 80), the proper input for the policy
        # this is required when building CNN with pytorch
        batch_input = preprocess_batch([fr1,fr2, fr3, fr4])
        ...
        fr1, re1, is_done, _ = envs.step(action)
        fr2, re2, is_done, _ = envs.step([0]*n)
        fr3, re3, is_done, _ = envs.step([0]*n)
        fr4, re4, is_done, _ = envs.step([0]*n)
