...
* After both training routines:
  * A plot is created that includes the max score per episode, the average score over the last 100 episodes, and the goal.
  * Ten tests are conducted and the score for each agent is printed
  * The model that achieves the goal is saved (an optional helper funtion to read in the saved model is also included)
